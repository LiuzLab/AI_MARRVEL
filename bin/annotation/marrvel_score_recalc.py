...
def omimCurate(vardf):
    ...
    medi_bool = ((vardf["omimVarFound"] == 1) & (vardf["omimSymMatchFlag"] == 0)) | (
        (vardf["omimVarFound"] == 0)
        & (vardf["omimGeneFound"] == 1)
        & (vardf["omimSymMatchFlag"] == 1)
    )
...
def hgmdCurate(vardf):
    ...
    medi_bool = ((vardf["hgmdVarFound"] == 1) & (vardf["hgmdSymMatchFlag"] == 0)) | (
        (vardf["hgmdVarFound"] == 0)
        & (vardf["hgmdGeneFound"] == 1)
        & (vardf["hgmdSymMatchFlag"] == 1)
    )
...
def clinvarCurate(vardf):
    ...
    one_bool = (vardf["clinVarSymMatchFlag"] == 1) & (
        (vardf["clinVarVarFound"] == 1) | (vardf["clinVarGeneFound"] == 1)
    )
    ...
    high_bool = (
        (vardf["hgmdVarFound"] == 1) & path_bool & (vardf["hgmdSymMatchFlag"] == 1)
    )
    medi_bool = (
        ((vardf["hgmdVarFound"] == 1) & path_bool & (vardf["hgmdSymMatchFlag"] == 0))
        | (
            (vardf["hgmdVarFound"] == 1)
            & (~path_bool)
            & (~bngn_bool)
            & (vardf["hgmdSymMatchFlag"] == 1)
            & (vardf["hgmdSymMatchFlag"] == 1)
