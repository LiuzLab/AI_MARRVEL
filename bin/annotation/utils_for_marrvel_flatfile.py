...
def getOMIMUsingMarrvelFlatFile(varObj, omimGeneList, omimAlleleList):
    ...
    if any(d["geneSymbol"] == varObj.geneSymbol for d in omimGeneList):
        ...
        geneFound = 1
        omimGeneDict = next(
            (sub for sub in omimGeneList if sub["geneSymbol"] == varObj.geneSymbol),
            None,
        )
...
def getVarObjFromMarrvelFlatFile(
    varDf,
    genomeRef,
    clinvarGeneDf,
    clinvarAlleleDf,
    ...
    for index, row in varDf.iterrows():
        ...
        if 1:
            ...
            omimRet = getOMIMUsingMarrvelFlatFile(
                varObjList[i], omimGeneList, omimAlleleList
            )
            ...
            print(
                "\tgeneFound:",
                varObjList[i].omimGeneFound,
                "varFound:",
                varObjList[i].omimVarFound,
            )
        ...
        if 1:
            ...
            clinVarRet = getClinVarUsingMarrvelFlatFile(
                varObjList[i], clinvarAlleleDf, clinvarGeneDf
            )
            ...
            if debugFlag == 1:
                print(
                    "\tgeneFound::",
                    varObjList[i].clinVarGeneFound,
                    "varFound:",
                    varObjList[i].clinVarVarFound,
                )
                print(
                    "\tnumVars:",
                    varObjList[i].clinvarTotalNumVars,
                    "numPathologic:",
                    varObjList[i].clinvarNumP,
                    "numBenign:",
                    varObjList[i].clinvarNumB,
                )
        ...
        if 1:
            ...
            print(
                "\thgmdVarFound:",
                varObjList[i].hgmdVarFound,
                "hgmdGeneFound:",
                varObjList[i].hgmdGeneFound,
                "hgmdVarPhenIdList:",
                varObjList[i].hgmdVarPhenIdList,
                "hgmdVarHPOIdList:",
                varObjList[i].hgmdVarHPOIdList,
                "hgmdVarHPOStrList:",
                varObjList[i].hgmdVarHPOStrList,
            )
        ...
        if 1:
            ...
            decipherRet = getDecipherUsingMarrvelAnnotScript(
                varObjList[i], decipherList
            )
            #
...
def getVarObjListFromDict(varDictList, numVars):
    ...
    for procList in varDictList:
        for varDict in procList:
            ...
            varObjList[i].clinvar_AlleleID = varDict[
                "clinvar_AlleleID"
            ]  # Clinvar allele ID from clinvar.vcf.gz
            varObjList[i].clinvar_clnsig = varDict[
                "clinvar_clnsig"
            ]  # CL: Clinvar SIG from clinvar.vcf.gz
            varObjList[i].clinvar_CLNREVSTAT = varDict[
                "clinvar_CLNREVSTAT"
            ]  # CL: Clinvar STAT from clinvar.vcf.gz, for interface only
            varObjList[i].clinvar_CLNSIGCONF = varDict[
                "clinvar_CLNSIGCONF"
            ]  # CL: Clinvar STAT from clinvar.vcf.gz
...
def getAnnotateInfo_2(
    ...
    for row in rowList:
        ...
        if "conserve" in moduleList:
            ...
            gnomadGeneMetricRet = getGnomadGeneMetricFlatFile(
                varObj, gnomadMetricsGeneDf
            )
            # [decipherDictList,decipherDeletionObsList,decipherStudyList, decipherVarFound]
