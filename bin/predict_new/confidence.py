...
def get_causal_distribution(pred_fd, label_col="is_strong"):
    ...
    top20_causals = np.ravel(
        top20_preds.loc[top20_preds[label_col] == 1, "predict"].to_numpy()
    )
    return top20_causals


def assign_confidence_score(causal_distr, pred_df, label_col="is_strong"):
    # pred_fd: folder for prediction files only
    ...
    conf_scores = [
        scipy.stats.percentileofscore(causal_distr, i) for i in prediction_scores
    ]
