...
def test(
    ...
    causal_rank_raw = rank_test_patients(
        final_model,
        val_fns,
        features,
        test_label,
        causal_distr=causal_distr,
        out_folder=f"{out_folder}/{mode}_pred_raw_rank",
        train_causals=None,
    )
...
def main(
    feature_fillna=None,
    feature_tier_with_clinvar=None,
    train_label=None,
    test_label=None,
    out_folder=None,
    tune=True,
    ...
    if out_folder is None:
        ...
        fd = "_".join(
            [classifier, fillna, tier, train_label, train_test, f"tune{tune}", dateid]
        )
    ...
    print(
        f"#fns={len(fns)}, #BG validation={len(bg_val_fns)}, #UDN validation={len(udn_val_fns)}, #DDD validation={len(ddd_val_fns)}"
    )
    ## Training files
    ...
    if tune:
        ...
        if classifier == "xgb":
            n_est, md, classifier = (
                int(val_best_params[0]),
                int(val_best_params[1]),
                float(val_best_params[2]),
            )
        else:
    ...
    if classifier == "xgb":
        ...
        final_model, features, train_causals = train_minibatch_RF(
            train_fns,
            n_estimator=n_est,
            max_depth=md,
    ...
    test(
        ...
        "UDN",
        train_causals,
        train_pred_folder=f"{out_folder}/BG_pred_raw_rank",
    )
    ### DDD validation
    test(
        final_model,
