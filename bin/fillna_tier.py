...
def feature_engineering(score_file, tier_file):
    ...
    patient["clinvarNumLP"] = (
        patient["clinvarNumLP"] + patient["clinvarNumP"]
    ) / patient["clinvarTotalNumVars"]
    ...
    patient["clinvarNumLB"] = (
        patient["clinvarNumLB"] + patient["clinvarNumB"]
    ) / patient["clinvarTotalNumVars"]
    ...
    patient.loc[
        patient["clin_code"].str.contains(
    ...
    patient.loc[patient["clin_PLP_perc"] != "-", "isP/LP"] = patient.loc[
        patient["clin_PLP_perc"] != "-", "clin_PLP_perc"
    ].astype("float64")
    ...
    patient.loc[patient["gnomadAFg"].isna(), "gnomadAFg"] = patient.loc[
        patient["gnomadAFg"].isna(), "gnomadAF"
    ]
    patient.loc[patient["gnomadAF"].isna(), "gnomadAF"] = patient.loc[
        patient["gnomadAF"].isna(), "gnomadAFg"
    ]
    ...
    if np.all(pd.isna(patient["phyloP100way_vertebrate"])):
        # Single variant query
        patient.loc[
            (pd.isna(patient["phyloP100way_vertebrate"])) & np.array(indel_index),
            "phyloP100way_vertebrate",
    ...
    if np.all(pd.isna(patient["REVEL_score"])):
        ...
        patient["REVEL_score"] = patient["REVEL_score"].astype("float64")
        patient.loc[
            (patient["REVEL_score"].isna()) & np.array(indel_index), "REVEL_score"
    ...
    # fathmm_MKL_coding_score
    patient.loc[
        patient["fathmm_MKL_coding_score"] == "-", "fathmm_MKL_coding_score"
    ] = np.NaN
    if np.all(pd.isna(patient["fathmm_MKL_coding_score"])):
        # Single variant query
        patient.loc[
            (pd.isna(patient["fathmm_MKL_coding_score"])) & np.array(indel_index),
            "fathmm_MKL_coding_score",
    ...
    # conservationScoreGnomad
    patient.loc[
        patient["conservationScoreGnomad"] == "-", "conservationScoreGnomad"
    ] = 1
    patient.loc[
        patient["conservationScoreGnomad"] == "Low", "conservationScoreGnomad"
    ] = 1
    patient.loc[
        patient["conservationScoreGnomad"] == "High", "conservationScoreGnomad"
    ] = 2
    patient["conservationScoreGnomad"] = patient["conservationScoreGnomad"].astype(
        "float64"
    )
    ...
    patient.loc[
        patient["conservationScoreOELof"] == "Low", "conservationScoreOELof"
    ] = 1
    patient.loc[
        patient["conservationScoreOELof"] == "High", "conservationScoreOELof"
    ] = 2
    patient["conservationScoreOELof"] = patient["conservationScoreOELof"].astype(
        "float64"
    )
    ...
    if np.all(pd.isna(patient["Polyphen2_HDIV_score"])):
        # Single variant query
        patient.loc[
            (pd.isna(patient["Polyphen2_HDIV_score"])) & np.array(indel_index),
            "Polyphen2_HDIV_score",
    ...
    if np.all(pd.isna(patient["Polyphen2_HVAR_score"])):
        # Single variant query
        patient.loc[
            (pd.isna(patient["Polyphen2_HVAR_score"])) & np.array(indel_index),
            "Polyphen2_HVAR_score",
    ...
    if np.all(pd.isna(patient["MutationAssessor_score"])):
        # Single variant query
        patient.loc[
            (pd.isna(patient["MutationAssessor_score"])) & np.array(indel_index),
            "MutationAssessor_score",
    ...
    for cons in consequence:
        patient["cons_%s" % cons] = (
            patient["Consequence"].str.contains(cons).astype("int")
        )
    ...
    for exp in c_clin_exp:
        ...
        patient["c_ClinVar_Exp_%s" % exp] = (
            patient["c_ClinVar_Exp"].str.contains(exp).astype("int")
        )
    ...
    for exp in c_hgmd_exp:
        ...
        patient["c_HGMD_Exp_%s" % exp] = (
            patient["c_HGMD_Exp"].str.contains(exp).astype("int")
        )
    ...
    for i in range(len(CLN_STAT)):
        patient.loc[patient["nc_CLNREVSTAT"] == CLN_STAT[i], "nc_CLNREVSTAT"] = (
            CLN_STAT_SCORE[i]
        )
        patient.loc[patient["c_CLNREVSTAT"] == CLN_STAT[i], "c_CLNREVSTAT"] = (
            CLN_STAT_SCORE[i]
        )
