...
def load_loc_maps(locmap):
    ...
    for line in open(locmap):
        ...
        if not by_symbol[chrom][symbol]:
            by_symbol[chrom][symbol] = [start, end]
        else:
            if by_symbol[chrom][symbol][0] > start:
                by_symbol[chrom][symbol][0] = start
            if by_symbol[chrom][symbol][1] < end:
                by_symbol[chrom][symbol][1] = end
