...
#* sample_ids.txt from rename_VCFheader.py
...
bcftools view -c1 -Oz -o /out/${1}.vcf.gz -s ${SAMPLE_P} /out/${1}.joint.vcf.gz

echo "Fix VCF formatting..."
(cat /trio_pipeline/acceptable_header.txt | grep -v '#CHROM'; zcat /out/${1}.vcf.gz | grep '#CHROM' | head -1; zcat /out/${1}.vcf.gz | grep -v '#' | awk 'BEGIN {FS = "\t"} {$7 = "PASS"; $8 = "."; gsub(":.*", "", $9); gsub(":.*", "", $10); gsub("\\|", "/", $10); print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" $9 "\t" $10}') | gzip > /out/${1}_fixed.vcf.gz

# Run marrvel singleton pipeline
echo "Run AIM default pipeline..."
#* make input file available for singleton script
# ? need to do "chmod 666 /out/${1}_fixed.vcf.gz; /input" before running the script?
echo "Copy fixed vcf file for singleton pipeline"
cp /out/${1}_fixed.vcf.gz /out/vcf.gz
# chmod 777 /out/${1}_fixed.vcf.gz
# cp /out/${1}_fixed.vcf.gz /input/vcf.gz
echo "Run singleton pipeline for proband"
#* run the singleton pipeline for proband
bash /run/proc.sh ${1} ${2} ${3} ${4}
...
python /trio_pipeline/merge_rm_trio.py ${1}
