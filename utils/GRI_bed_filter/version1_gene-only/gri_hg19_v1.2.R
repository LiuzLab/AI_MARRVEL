...
# 1. Use gene info from GENCODE (hg19) for GRI ----------------------------
#read gtf file from gencode: https://www.gencodegenes.org/human/release_46lift37.html
gtf_file <- "gencode.v46lift37.basic.annotation.gtf"
gtf_data <- rtracklayer::import(gtf_file)
gtf_dt <- as.data.table(as.data.frame(gtf_data))
##include the hg19 chromosome ending positions: https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_000001405.25/
...
bed_coding <- genes |> 
  ...
  mutate(
    source = "gencode.v46lift37.protein_coding"
  )
...
# 2. Use HGMD 2022 (hg19) for GRI -----------------------------------------
#read hgmd vcf file
vcf_hg19<-'HGMD_Pro_2022.2_hg19.vcf'
vcf_data <- readVcf(vcf_hg19, genome = "hg19")
...
# 3. Include ClinVar (hg19) for GRI ----------------------------------------
#https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/
#process vcf data
vcf_hg19<-'clinvar.vcf'
vcf_data <- readVcf(vcf_hg19, genome = "hg19")
...
bed_clinvar <- clinvar_patho_size_flank %>% 
  ...
  mutate(source="Clinvar_hg19_080624")
##merge and reduce
...
#write the bed file
write.table(bed_gencode_hgmd_clinvar.dt[,c(1:3)], file='genomic_region_filter.hg19.v1.bed', sep="\t", quote=FALSE, col.names = FALSE, row.names=FALSE)
